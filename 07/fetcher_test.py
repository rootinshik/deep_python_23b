...
class TestFetcher(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fetch_all_urls(self, mock_print, mock_get):
        with patch("fetcher.open",
                   mock_open(read_data=TestFetcher.get_num_links(100))) as _:
            response = [f"link_{i}" for i in range(100)]
            mock_get.return_value.__aenter__.return_value.text.side_effect = response
            print_args = []
            ...
            mock_get.assert_has_calls(
                [call(link) for link in TestFetcher.get_num_links(100).split("\n")],
                any_order=True
            )
    ...
    async def test_different_num_urls(self, mock_print, mock_get):
        for num_urls in [1, 10, 25, 100, 1_000]:
            with patch("fetcher.open",
                       mock_open(read_data=TestFetcher.get_num_links(num_urls))) as _:
                ...
                mock_get.assert_has_calls(
                    [call(link) for link in TestFetcher.get_num_links(num_urls).split("\n")],
                    any_order=True
                )
                self.assertEqual([resp + ": {}" for resp in response], print_args)
    ...
    async def test_different_num_workers(self, mock_print, mock_get):
        for num_workers in [1, 10, 25, 100, 1_000]:
            with patch("fetcher.open",
                       mock_open(read_data=TestFetcher.get_num_links(10))) as _:
                ...
                mock_get.assert_has_calls(
                    [call(link) for link in TestFetcher.get_num_links(10).split("\n")],
                    any_order=True
                )
                self.assertEqual([resp + ": {}" for resp in response], print_args)
    ...
    async def test_fetch_worker(self, mock_print, mock_get):
        ...
        self.assertEqual([f"link_{i}" + ': {"test": 1}' for i in range(3)], print_args)
