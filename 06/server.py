...
class Server:
    def __init__(self, num_workers: int, k_freq_words: int):
        ...
        self.workers = [Worker(self.clients_queue,
                               self.k_freq_words,
                               self)
                        for _ in range(self.num_workers)]
        self.master = Master(self.clients_queue)

    def start(self):
        print(f"Starting server with {self.num_workers} "
...
class Master(threading.Thread):
    def __init__(self, clients_queue: Queue, *args, **kwargs):
